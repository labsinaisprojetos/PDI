...
for i in range(scaledImg.shape[0]):
    for j in range(scaledImg.shape[1]):
        for h in range(scaledImg.shape[2]):
            if scaledImg[i][j][h] > 1:
                scaledImg[i][j][h] = 1
...
cv2.imwrite('Brightness_scaled.png', scaledImg*255)
